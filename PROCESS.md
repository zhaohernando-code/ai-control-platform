...
## 2026-05-25 根域 agent auth probe fallback

- Problem: the root-domain edge could authenticate agent requests with `X-HZ-Dev-Auth-Bypass-Token`, but `probe-workbench-live-route.mjs` still depended on the current Codex CLI process environment containing `HZ_DEV_AUTH_BYPASS_TOKEN`. Fresh CLI sessions therefore reported the bare browser login redirect and could not produce authenticated pass evidence.
- Resolution: `probe-workbench-live-route.mjs` now auto-injects the edge agent auth header for `https://hernando-zhao.cn/*` routes when no explicit Cookie/Authorization/auth header is supplied. It first uses `HZ_DEV_AUTH_BYPASS_TOKEN`; if absent, it calls `/Users/hernando_zhao/codex/scripts/edge-agent-auth-token.sh`. The evidence artifact records only `request_header_names`, never the token value.
- Prevention: future live-route checks must distinguish bare-public browser protection from authenticated agent verification. A naked 302 remains expected; closeout evidence should be generated by the probe with agent auth fallback.
