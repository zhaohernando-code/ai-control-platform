...
- Workbench server 提供 `POST /api/workbench/workbench-browser-events-run`，校验并持久化 browser events artifact 到 manifest 与 artifact ledger，后续 scheduler cycle 可以直接读取 projection 中的 UI verification 证据。
