...
test("current session workbench fixture is generated from durable input", () => {
  ...
  assert.equal(projection.manifest.event_count, expectedProjection.manifest.event_count);
  assert.equal(projection.one_screen.counters.artifacts, expectedProjection.one_screen.counters.artifacts);
  ...
  assert.equal(projection.reviewer_shard_review.status, "pass");
  ...
  assert.equal(projection.next_action_readout.action, expectedProjection.next_action_readout.action);
});
