...
| 资源 | 用途 | 迁移期处理 |
| --- | --- | --- |
| `apps/workbench/styles.css` | 自造原生 CSS（约 930 行） | 默认不保留；除非领域语义无法用 antd token 表达 |
...
`tools/check-workbench-browser-events.mjs` 与 `tools/check-workbench-frontend-acceptance.mjs`
按上述 key 做 DOM 验证；新前端必须在等价 antd 组件上保留同样的 key 或提供等价的
data-test attribute，并同步更新这两个 check 脚本（不得弱化断言）。
