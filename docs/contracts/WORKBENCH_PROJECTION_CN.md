...
- Project Management：当前纳管项目、阶段、当前任务、负责人 Agent、项目任务流和项目级计数。
...
| `project_management` | 当前项目列表、active work、任务流、项目级 counters 和设计对齐信号 |
...
| `next_action_terminal` | 当推荐动作不是 ready 时记录 terminal action / reason，供 CLI loop 和 PC/mobile 判断为何停机 |
...
`project_management` 是 PC / mobile 工作台的产品主语，合同见 `docs/contracts/PROJECT_MANAGEMENT_WORKBENCH_CN.md`。Workbench Projection 可以继续暴露 scheduler、reviewer、artifact ledger 等诊断层，但前端不能把这些诊断层作为首屏主内容替代项目列表、项目阶段和项目任务流。

`next_action_readout` 是工作台和调度器共享的推荐动作读出，不是执行授权本身。实际执行必须走 `/api/workbench/next-action`，由服务端重新计算 projection、校验 `expected_action`，并只执行已接入的白名单动作。当前已接入动作为 `enqueue_scheduler_next_cycle`、`run_autonomous_scheduler_loop`、`run_reviewer_scope_shard`、`continue_after_reviewer_aggregate`、`prepare_project_status_continuation`、`create_context_pack_from_seed`、`run_context_work_packages`。

`next_action_terminal` 是 `next_action_readout` 的停机证据层。当 `next_action_readout.status=ready` 时，`terminal_action` 和 `terminal_reason` 必须为空；当状态为 `pending`、`blocked`、`not_configured` 或其他非 ready 状态时，projection 必须把推荐的停机动作与原因复制到 `terminal_action` / `terminal_reason`。PC/mobile schema 必须要求该对象存在，工作台 shell 和 browser closeout 必须渲染它，避免 projected loop 停在 `inspect_*` 或 `wait_for_driver_event` 时只靠 transient phase、stdout 或 raw manifest 解释。
...
当没有 scheduler/reviewer/recovery 等更具体的 automation driver，但 `global_goal_completion.status=in_progress` 时，`next_action_readout` 必须推荐 `prepare_project_status_continuation`。该动作通过 `/api/workbench/next-action` 间接执行 `/api/workbench/project-status-continuation`，把仓库 `PROJECT_STATUS` 的 continuation decision、next work packages 和 `context_pack_seed` 写回 workflow state，事件类型为 `project_status_continuation`。如果已经存在 reviewer shard、scheduler continuation、resume 等更具体 driver，则它们优先于 global-goal fallback，避免全局目标抢占正在恢复的具体任务。

当最新 automation driver 是完成态 `reviewer_shard_aggregate` 时，`next_action_readout` 必须推荐 `continue_after_reviewer_aggregate`。该动作通过 `/api/workbench/next-action` 间接执行 `/api/workbench/project-status-continuation`，把 aggregate 的 `merged_findings` 重新纳入 continuation decision。写入 `project_status_continuation` 后，该最新 workflow fact 必须接管读数并推荐 `create_context_pack_from_seed`，不能被旧 aggregate driver 反复抢占。
当 `project_status_continuation` 的 `context_pack_seed` 来自 reviewer aggregate continuation 时，它必须以清理后的 recovery 事实为准，不得把 reviewer provider health、timeout recovery 或旧 shard result 恢复包继续带入下一轮 seed。

当最新 workflow fact 是 ready 的 `project_status_continuation` 时，`next_action_readout` 必须推荐 `create_context_pack_from_seed`。该动作通过 `/api/workbench/context-pack-cycle` 校验 `context_pack_seed`、生成新的 Run Manifest / Work Packages / Artifact Ledger，并发布下一轮 projection-ready workflow snapshot。materialized 后的 source workflow 必须写入 `context_pack_cycle_materialized` fact；新 cycle 必须包含 `context_pack_cycle_created` fact。

当最新 workflow fact 是 `context_pack_cycle_materialized`、`context_pack_cycle_created` 或 `context_work_packages_run`，且 `task_dag.dispatchable` 非空时，`next_action_readout` 必须推荐 ready 的 `run_context_work_packages`。该动作通过 `/api/workbench/context-work-packages-run` 执行有界工作包批次，写回 `context_work_packages_run` event/artifact，并更新 Run Manifest work package 状态与 Task DAG。执行后如果仍有可派发节点，readout 继续推荐 `run_context_work_packages`；如果没有可派发节点但 `global_goal_completion.status=in_progress`，readout 必须回到 `prepare_project_status_continuation`；只有在工作包已耗尽且没有待继续的全局目标时，才进入 `inspect_context_work_packages` pending 状态。
...
- project management 的项目计数、项目列表和任务流。
...
- terminal next action 的 status、terminal action、terminal reason。
...
- PC projection 必须包含 manifest、artifacts、closeout、resume health、reviewer provider health、scheduler dispatch、scheduler loop、project management、global goal completion、operations timeline、next action readout、model routing、reviewer gate、autonomous run、task DAG 和 one-screen 摘要。
- Mobile projection 必须包含 headline、counters、next actions、closeout、project management、resume health、provider health、scheduler dispatch、scheduler loop、global goal completion、operations timeline、next action readout、model 和 reviewer 摘要。
