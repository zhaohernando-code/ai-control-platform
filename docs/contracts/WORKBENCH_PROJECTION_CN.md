...
当最新 workflow fact 是 ready 的 `project_status_continuation` 时，`next_action_readout` 必须推荐 `create_context_pack_from_seed`。该动作通过 `/api/workbench/context-pack-cycle` 校验 `context_pack_seed`、生成新的 Run Manifest / Work Packages / Artifact Ledger，并发布下一轮 projection-ready workflow snapshot。materialized 后的 source workflow 必须写入 `context_pack_cycle_materialized` fact；新 cycle 必须包含 `context_pack_cycle_created` fact，后续执行器未接入前，readout 可以停在 `run_context_work_packages` pending 状态。
