...
`next_action_readout` 是工作台和调度器共享的推荐动作读出，不是执行授权本身。实际执行必须走 `/api/workbench/next-action`，由服务端重新计算 projection、校验 `expected_action`，并只执行已接入的白名单动作。
...
- PC projection 必须包含 manifest、artifacts、closeout、resume health、reviewer provider health、scheduler dispatch、scheduler loop、operations timeline、next action readout、model routing、reviewer gate、autonomous run、task DAG 和 one-screen 摘要。
- Mobile projection 必须包含 headline、counters、next actions、closeout、resume health、provider health、scheduler dispatch、scheduler loop、operations timeline、next action readout、model 和 reviewer 摘要。
