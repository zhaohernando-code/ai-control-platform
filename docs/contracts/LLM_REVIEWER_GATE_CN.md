...
DeepSeek/Claude Code executor adapter 约束：

- 必须通过 `/Users/hernando_zhao/.codex/skills/claude-deepseek-review/scripts/run_claude_deepseek_review.py` 调用用户的 canonical launcher `/Users/hernando_zhao/codex/start-claude-deepseek-no-proxy.sh`。
- 不直接编辑 launcher。
- shard.allowed_tools 为空时传 `--tools ""`；只读 shard 可传 `Read,Grep,Glob` 等只读工具。
- 默认模型为 `deepseek-v4-pro[1m]`，timeout 来自 shard/profile。
