...
中台不应固定使用某一个最高级模型，也不应把 agent invocation 当作临时人工习惯。模型选择必须成为可审计、可测试、可展示的调度策略。
...
| `model_routing_strategy` / `ds_participation_mode` | `balanced`、`ds_expanded`、`cost_saving`；用于临时提高或恢复 DS 参与比例 |
| `ds_ratio_boost` | 1 到 5 的数值，记录 DS 参与增强强度，不能靠会话口头约定 |
...
- 当 `codex_plan_pressure` 出现且未显式配置策略时，路由策略默认为 `ds_expanded`。在该策略下，低/中风险 `context_pack`、`planning`、`implementation` 可以由 `deepseek-v4-pro` 作为 primary，并由 `deepseek-v4-flash` 先做 scout 压缩上下文；这是可配置策略，压力解除后可回到 `balanced`。
- `ds_expanded` 不得让 Recovery 取得 DS primary ownership。`recovery` stage、`recovery` tag、关键恢复、回滚、安全、架构、破坏性操作、最终仲裁、高风险 `platform_core` 和 `critical` 风险仍必须由 `gpt` 主导，DS 只能作为 scout / process_guard / independent_reviewer。
...
- bounded real reviewer 默认只允许 1 次外部 DS 调用、120 秒 timeout。`ds_expanded` 可临时放宽到 2 次外部 DS 调用，但累计 wall-clock 预算必须不超过 240 秒，避免 plan 压力下用更长等待换取“看起来更多的 DS 参与”。
...
- 平台不再依赖手动 `个人 DeepSeek launcher` 或 reviewer wrapper。健康诊断必须基于项目内 channel/profile/key store：区分“provider 通道可用”“profile 调用参数错误”“key 不健康/被锁定”和“非结构化输出”。
...
- 当前默认真实 executor 是 `agent_invocation_provider_executor`，由 `context-work-package-provider-executor` 通过项目内 `agent-invocation` profile/channel/key-store 启动。它必须记录 bounded timeout、model、tools/no-tools、cwd、budget、effort、profile、agent id、command runner kind 和外部调用次数；生产默认路径只能使用真实 `spawn_sync` command runner 或不含 fake/test/mock/simulation/local/deterministic token 的外部 provider runner。
...
- model routing strategy 与 ds ratio boost。
- role count。
- 每个模型承担的角色数量。
- DS/GPT role count 与 DS 是否是 primary。
- 是否有 process guard。
