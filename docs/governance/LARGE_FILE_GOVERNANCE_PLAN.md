...
| LFG-P6.2 | Retire unused static code | `tools/check-workbench-next-served-route.mjs`; `docs/examples/workbench-next-served-route-evidence-20260602.json`; remaining deletions blocked | Next.js served-route gate now passes for mounted desktop/mobile App Router runtime; legacy browser-events, frontend-acceptance, scheduler writeback, and shell-test dependencies still block deletion/quarantine. | blocked |
...
| LFG-P6 | blocked | Next.js served-route gate now passes at `/projects/ai-control-platform/` for desktop/mobile mounted App Router runtime, API rewrite, favicon, all eight main App Router routes, SPA navigation, and no legacy shell. Legacy `workbench.js` and `styles.css` still remain explicit fallback and acceptance-gate dependencies for browser-events, frontend-acceptance, scheduler writeback browser verification, and workbench shell tests. | DeepSeek PASS for P6.1 inventory and P6.2 partial served-route replacement; remaining interaction/content/writeback gate migration still blocks deletion |
