...
| LFG-Q06 | `test/frontend-acceptance.test.js` | 1671 | `planned_refactor` | Split content, browser error, route, and command-architecture validators. |
...
| LFG-Q18 | `src/workflow/frontend-acceptance.js` | 818 | `planned_refactor` | Extract acceptance subvalidators and browser-evidence parsing. |
...
| LFG-P6.3 | Migrate remaining legacy-dependent gates | Full browser-events closeout, scheduler writeback browser verification, and shell tests no longer depend on `desktop.html`/`mobile.html`/`workbench.js`/`styles.css` | Full browser-events closeout now replays the 15 required scenarios through the mounted Next.js App Router runtime, and scheduler dispatch writeback now verifies desktop/mobile readouts through the mounted Next.js App Router runtime. Legacy static deletion remains blocked until shell gates replay equivalent behavior through Next.js or are replaced by stronger runtime gates. | in_progress |
| LFG-P6.4 | Quarantine/delete legacy static assets and update manifest | `.largefile-manifest.json`; deleted/quarantined legacy static entries | Removed/retired files no longer appear as planned-refactor entries only after P6.3 passes. | pending |
| LFG-P6.5 | DeepSeek phase review | Reviewer artifact | DS confirms no public route or compatibility fixture was broken and no legacy-static false-pass remains. | pending |
...
| LFG-P6 | in_progress | Next.js served-route gate now passes at `/projects/ai-control-platform/` for desktop/mobile mounted App Router runtime, API rewrite, favicon, all eight main App Router routes, SPA navigation, and no legacy shell. Primary frontend-acceptance now runs through `tools/check-workbench-next-frontend-acceptance.mjs` against the mounted App Router runtime and keeps the existing artifact validator/durable release evidence. `tools/check-workbench-browser-events.mjs` now runs the full 15-scenario browser-events closeout through mounted Next.js App Router and mounted API rewrite, writes `route_family: nextjs_app_router`, `legacy_static_shell_used: false`, and `legacy_interactions_replayed: true`, and passes the existing closeout artifact validator. `tools/check-scheduler-dispatch-writeback.mjs` now writes scheduler dispatch facts through the Workbench API service and verifies desktop/mobile scheduler readouts through mounted Next.js App Router with `legacy_static_shell_used: false`. Legacy `workbench.js` and `styles.css` still remain explicit dependencies for workbench shell tests and static opt-in quarantine. | DeepSeek PASS for P6.1 inventory and P6.2 served-route replacement; DeepSeek blocked the earlier partial browser-events migration claim and the scope was reduced to a writeback probe; full browser-events migration passed DeepSeek after focused follow-up confirmed release-default high-risk controls are gated; scheduler writeback migration passed DeepSeek; shell gate migration still blocks deletion |
