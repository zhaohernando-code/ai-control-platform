...
| LFG-P6 | blocked | Next.js served-route gate now passes at `/projects/ai-control-platform/` for desktop/mobile mounted App Router runtime, API rewrite, favicon, all eight main App Router routes, SPA navigation, and no legacy shell. Primary frontend-acceptance now runs through `tools/check-workbench-next-frontend-acceptance.mjs` against the mounted App Router runtime and keeps the existing artifact validator/durable release evidence. A separate `tools/check-workbench-next-browser-events.mjs` probe writes `workbench-browser-events-run.v1` through the mounted Next API, but full browser-events closeout remains on `tools/check-workbench-browser-events.mjs` because Next UI does not yet replay legacy lifecycle cleanup/timeout interactions. Legacy `workbench.js` and `styles.css` still remain explicit dependencies for browser-events closeout, scheduler writeback browser verification, and workbench shell tests. | DeepSeek PASS for P6.1 inventory and P6.2 served-route replacement; DeepSeek blocked full browser-events migration claim and the scope was reduced to a Next writeback probe; DeepSeek PASS AFTER DELTA for Next frontend-acceptance migration; remaining browser-events/writeback/shell gate migration still blocks deletion |
